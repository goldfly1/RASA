            ...
            with ui.column().classes("flex-1 min-w-0 relative"):
                ...
                with ui.tab_panels(tabs, value=tab_value).classes("w-full flex-1 bg-gray-900 text-white overflow-auto"):
                    ...
                    with ui.tab_panel(market_tab):
                        wrap = ui.scroll_area().classes("w-full h-full")
                        with wrap:
                            with ui.column().classes("w-full"):
                                if cell and cell.system:
                                    market = game.get_market(session.player_id)
                                    if market:
                                        ui.label("🛒 Market").classes("text-sm font-bold text-amber-300 q-mt-sm q-mb-xs")
                                        for good_key, info in market.items():
                                            ...
                                            with ui.row().classes("w-full items-center gap-1 text-sm py-1 border-b border-gray-700"):
                                                ...
                                                with ui.column().classes("items-end"):
                                                    ...
                                                    ui.button("Buy", on_click=lambda _, gk=good_key, qs=qty_select: buy_good(session, gk, int(qs.value or 1))).props("size=sm flat color=green")
                                    else:
                                ...
                                if ship and ship.cargo_hold:
                                    ui.label("📦 Cargo").classes("text-sm font-bold text-gray-300 q-mt-sm q-mb-xs")
                                    for item in ship.cargo_hold:
                                        ...
                                        with ui.row().classes("w-full items-center gap-1 text-sm py-1 border-b border-gray-700"):
                                            ...
                                            with ui.column().classes("items-end"):
                                                ...
                                                ui.button("Sell", on_click=lambda _, gk=item.good_key, qs=qty_sel: sell_good(session, gk, qs.value)).props("size=sm flat color=orange")
                    # ── Intel Tab ──────────────────────────────────────
